...
<img src="docs/screenshot-search.png" width="330" alt="The search tab of the floating panel: the day complete at 60 of 60 points, with the start button still offered to search anyway and a line saying how long ago the Rewards data was read; below it the journey counter, the puzzle of stamps with the bonus for completing it, and one line per check-in partner with the days already earned of its seven-step streak and what the last step pays — the ones you can advance first, and greyed out below them the three that are not available on this account; then links to the other Rewards activities, to the missions Outlook shows only inside itself and to the tasks Xbox runs on its own; at the bottom, the points balance converted to Xbox credit">

*Search tab: the day's progress in points as Rewards counts it, the controls for the current state, the day's tasks — your streak, each check-in partner with the days already earned, and the daily set with a link to every activity still pending — and what your points are worth. Streaks that are not available on your account stay in the list, greyed out and at the bottom: they are points that exist, just not ones you can chase from here. / Pestaña de búsqueda: el progreso del día en puntos, tal como los cuenta Rewards, los controles según el estado, las tareas del día —tu racha, cada socio del check-in con los días que ya lleva ganados, y el conjunto diario con enlace a cada actividad que falte— y cuánto valen tus puntos. Las rachas que no están disponibles en tu cuenta siguen en la lista, en gris y al final: son puntos que existen, solo que no se pueden perseguir desde aquí.*
...
- At the bottom, **links to the two places this panel cannot read**. Xbox runs its own daily, weekly and monthly tasks from the app or the console, and Outlook shows point missions inside itself that change from one day to the next; neither shows up here nor in the Rewards dashboard. Nothing is claimed about how many or how much: the links just say where to look.
...
- Al final, **enlaces a los dos sitios que este panel no puede leer**. Xbox tiene sus propias diarias, semanales y mensuales desde la app o la consola, y Outlook enseña dentro de sí mismo misiones de puntos que cambian de un día a otro; ninguna de las dos cosas sale aquí ni en el panel de Rewards. No se afirma cuántas son ni cuánto pagan: los enlaces solo dicen dónde mirar.
